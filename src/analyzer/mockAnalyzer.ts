...
/**
 * Tipo do sinal como a TELA o conhece.
 *
 * A interface do responsável escolhe o ícone de cada sinal por este nome
 * (`secrecy_request` -> cadeado, `image_request` -> câmera…). Emitir a chave
 * interna da característica faria todo sinal cair no ícone genérico.
 */
...
function buildSignals(hits: PatternHit[]): DetectedSignal[] {
  return hits.map((hit, index) => {
    ...
    return {
      ...
      type: SIGNAL_TYPE[hit.key] ?? hit.key,
